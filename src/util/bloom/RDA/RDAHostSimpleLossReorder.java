...
public class RDAHostSimpleLossReorder {
	...
	//constructor
	public RDAHostSimpleLossReorder() {
	...
	public void parseFiles(){
		 ...
		 _hostReceiver.clear();
		//set the parameters
		setParamsPacketTrace(scale,shape,dropProb,reorderProb);
		...
		try {
			...
			long lines=0;
		      ...
		      while (sc.hasNext()&&lines<=TotalPackets) {
		        ...
		        double[] rec = pg.NextWeibullPacket(timestamp);
		        int state = pg.dropOrReorder();
		        // log.main("state: "+state+", ts: "+timestamp+", id: "+id[0]);
		        if(state==0){
					//only sender records
		        	 //record the packets
			         _hostSender.packetIncoming(id[0],rec[1]);			
					continue;
				}else if(state==1){
					//loss				
					_hostReceiver.packetIncoming(id[0],rec[2]);
					continue;
				}
				else{
				//good	
				...
				AkMinusOne = Ak;
				xk=Math.abs(rec[2]-rec[1]);
				//average
				Ak=onePassAvg(i+1,Ak,xk);
				//standard deviation
	...
	public double[] computeStatistics(RDATable  receiver){
		double avg = _hostSender.getAverage(receiver);
	...
	public static void main(String[] args){
		...
		RDAHostSimpleLossReorder test=new RDAHostSimpleLossReorder();
		test.begin();
