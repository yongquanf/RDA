 ...
 * 
 */
public class NetAddressOld implements AddressIF, Serializable {
	protected static final Log log = new Log(NetAddressOld.class);
	...
	NetAddressOld(String hostString, CB1<AddressIF> cbAddress) {
		this(hostString, null, UNKNOWN_PORT, cbAddress);
	}
	
	NetAddressOld(String hostString, int port, CB1<AddressIF> cbAddress) {
		this(hostString, null, port, cbAddress);		
	}

	NetAddressOld(int port) throws UnknownHostException {
		this(InetAddress.getLocalHost().getCanonicalHostName(),
					InetAddress.getLocalHost().getAddress(), port, null);
	}

	NetAddressOld(final String hostString, byte[] byteIPAddr, final int port,
			...
			resolve(new CB0() {
				...
				protected void cb(CBResult result) {
					switch (result.state) {
						case OK: {
								try {
									if (NetAddressOld.this.port != UNKNOWN_PORT) {									
										NetAddressOld.this.inetSocketAddress = new InetSocketAddress(
												InetAddress.getByAddress(NetAddressOld.this.byteIPAddr), NetAddressOld.this.port);										
									} else {
										log.debug("Created address without port: hostname=" + hostname);
										NetAddressOld.this.inetAddress = InetAddress.getByAddress(hostname, NetAddressOld.this.byteIPAddr);
									}										
								} catch (UnknownHostException e) {
									String ip = null;
									if (NetAddressOld.this.byteIPAddr != null)
										ip = NetUtil.byteIPAddrToString(NetAddressOld.this.byteIPAddr);
									log.error("Resolved address incorrectly. host=" + hostString + " ip=" + ip
											+ " port=" + NetAddressOld.this.port + ": " + e);
								}
							break;
						}
						case TIMEOUT:
					...
					cbAddress.call(result, NetAddressOld.this);
				}
			});
		} else {
					...
					switch (result.state) {
						case OK: {
							log.debug("Hostname resolved: " + NetUtil.byteIPAddrToString(byteIPAddr) + "->"
									+ hostname);
							NetAddressOld.this.hostname = hostname;
							cb.call(result);
							break;
						}
						case TIMEOUT:
				...
				protected void cb(CBResult result, byte[] byteIPAddr) {
					switch (result.state) {
						case OK: {
							...
							NetAddressOld.this.byteIPAddr = byteIPAddr;
							cb.call(result);
							break;
						}
						case TIMEOUT:
	...
	public boolean equals(Object obj) {
		assert obj instanceof NetAddressOld : "Cannot compare NetAddress to non-NetAddress.";

		NetAddressOld cmpAddress = (NetAddressOld) obj;
	...
	public static void main(String[] args) {		
	   ...
	   NetAddressOld ip = ( NetAddressOld )AddressFactory.createUnresolved("202.197.22.56", 16);
							...
							case OK: {
								...
								for (String addrStr : addressMap.keySet()) {
									...
									log.main("address=" + addrStr + " resolvedAddr=" + addr + " address.byteIP="
											+ NetUtil.byteIPAddrToString(((NetAddressOld) addr).getByteIPAddr()));
								}
