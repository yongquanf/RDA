...
public class NetAddress implements AddressIF, Serializable {
	...
	protected InetAddress iNetIPAddress;
	...
	protected NetAddress(final String hostString, byte[] byteIPAddr, final int port,
		...
		cbAddress.call(CBResult.OK(), NetAddress.this);
		}else{
			try {
				...
				if (NetUtil.byteIPToIntIP(this.byteIPAddr) != 0) {
					if (port != UNKNOWN_PORT) {					
						...
						//this.iNetIPAddress = InetAddress.getByAddress(hostname, byteIPAddr);						
					}						
				} else {					
					...
					switch (result.state) {
						case OK: {
							log.debug("Hostname resolved: " + NetUtil.byteIPAddrToString(byteIPAddr) + "->"
									+ hostname);
							NetIPAddress.this.hostname = hostname;
							cb.call(result);
							break;
						}
						case TIMEOUT:
				...
				protected void cb(CBResult result, byte[] byteIPAddr) {
					switch (result.state) {
						case OK: {
							...
							NetIPAddress.this.byteIPAddr = byteIPAddr;
							cb.call(result);
							break;
						}
						case TIMEOUT:
...
*/
	...
	public InetAddress getINetIPAddress() {
		assert inetSocketAddress != null || iNetIPAddress != null;		
		return inetSocketAddress != null ? inetSocketAddress.getAddress() : iNetIPAddress;		
	}
	...
	public boolean equals(Object obj) {
		assert obj instanceof NetAddress : "Cannot compare NetIPAddress to non-NetIPAddress.";
	...
	public String toString(boolean colour) {
		...
		if (colour)
			return ANSI.color(ADDR_COLOR, ipStr + addPort);

		return ipStr + addPort;
	}
